...
При таймауте записи сервис повторно читает маршрут из контроллера: подтвержденное совпадение считается успешным обновлением. Устаревший локальный hash не вызывает повторную запись уже примененного списка. Таймаут API по умолчанию — 120 секунд; для медленных контроллеров и крупных списков его можно увеличить через `unifi.timeout`.

## Релизы

[v0.3: изменения и переход с v0.2](docs/releases/v0.3.md). Версионный образ: `ghcr.io/dexogen/iplist-go-unifi:v0.3`.

Для выпуска добавьте `docs/releases/<tag>.md`, затем отправьте тег формата `vX.Y` или `vX.Y.Z`. Workflow `Build image` проверит описание, выполнит тесты и проверку запуска контейнера, опубликует образы amd64/arm64 и создаст GitHub Release с digest образа. Уже опубликованные версии не переиспользуются.
